...
const primarySlot = document.querySelector('.vote-slot.primary');
const secondarySlot = document.querySelector('.vote-slot.secondary');
...
function showResultBanner(winner) {
  ...
  resultQuestion.textContent = state.currentQuestion || '';
...
function clearSuspense() {
  ...
  fusionCore.classList.remove('winner');
  rouletteImage.src = '';
  ...
  const spin = () => {
    ...
    if (delay < maxDelay) {
      ...
      const finale = setTimeout(() => {
        ...
        fusionCore.classList.add('winner');
...
function handleJoined(payload) {
  ...
  if (payload.currentRound && payload.currentRound.headIds) {
    ...
    if (state.roundActive) {
      ...
      renderPodium(results);
      showResultBanner(results[0] || null);
