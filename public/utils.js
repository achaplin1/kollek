...
const waveClueSelect = document.getElementById('wave-clue-select');
...
const waveTarget = document.getElementById('wave-target');
const waveGuessBand = document.getElementById('wave-guess-band');
...
function setHeroForGameIntro(gameType) {
  if (gameType === 'wavelength') {
    setHeroForWave('Prêt à lancer une manche ?', 'Sélectionne le donneur d\'indice et prépare ton hint.');
...
function renderWaveAvatars(guesses = [], revealed = false) {
  ...
  guesses
    ...
    .forEach((entry) => {
      ...
      node.style.setProperty('--wave-pos', `${entry.value}`);
...
function renderWaveState() {
  ...
  switch (wave.status) {
    ...
    case 'revealed':
      ...
      break;
    default:
      setHeroForWave('Prêt à lancer une manche ?', 'Choisis qui donne l\'indice.');
      break;
  }
  ...
  if (waveSlider) {
    waveSlider.value = `${wave.guessValue || 50}`;
    waveSlider.disabled = !canGuess || wave.guessLocked;
  }
  ...
  if (waveTarget) {
    if (wave.target && (wave.status === 'revealed' || isClueGiver)) {
      const width = Math.max(2, (wave.target.end || 0) - (wave.target.start || 0));
      waveTarget.style.display = 'block';
      waveTarget.style.left = `${Math.max(0, wave.target.start || 0)}%`;
      waveTarget.style.width = `${Math.min(100, width)}%`;
    } else {
      waveTarget.style.display = 'none';
    }
  }
  ...
  if (waveGuessBand) {
    if ((wave.status === 'locked' || wave.status === 'revealed') && wave.guesses.length) {
      ...
      waveGuessBand.style.display = 'block';
      waveGuessBand.style.left = `${min}%`;
      waveGuessBand.style.width = `${Math.max(2, max - min)}%`;
    } else {
      waveGuessBand.style.display = 'none';
    }
  }
  ...
  if (wave.status === 'waitingClue') {
    ...
    hintText = 'Choisis un donneur d\'indice et lance la manche.';
  }
  ...
  if (wave.status === 'guessing' && wave.waiting) {
    ...
    if (wave.status === 'waitingClue') {
      ...
      statusText = 'Choisis un donneur d\'indice et lance la manche.';
    }
    if (waveStatus) {
...
function renderPlayers(players = []) {
  ...
  if (waveClueSelect) {
    waveClueSelect.innerHTML = '';
    players.forEach((player) => {
      const option = document.createElement('option');
      option.value = player.id;
      option.textContent = player.name;
      waveClueSelect.appendChild(option);
    });
    if (state.wave.clueGiverId) {
      waveClueSelect.value = state.wave.clueGiverId;
    } else if (state.isAdmin && waveClueSelect.options.length) {
      waveClueSelect.value = waveClueSelect.options[0].value;
    }
  }
  renderWaveScoreboard();
  if (
...
function handleGameSelected({ gameType, heads = [], questions = [], waveState = null, waveScores = [] }) {
  ...
  if (gameType === 'qui') {
    ...
    setRoundHint('Choisis un donneur d\'indice et lance la manche.');
  }
}
...
function handleWaveRoundPending({ clueGiverId, card, roundNumber }) {
  ...
  state.wave.waiting = null;
  if (waveClueSelect) {
    waveClueSelect.value = state.wave.clueGiverId || waveClueSelect.value;
  }
...
if (waveStartRoundButton) {
  waveStartRoundButton.addEventListener('click', () => {
    ...
    const clueGiverId = waveClueSelect ? waveClueSelect.value : state.selfId;
    if (!clueGiverId) {
      showToast('Choisis un joueur pour donner l\'indice.');
      return;
    }
...
if (waveClueSelect) {
  waveClueSelect.addEventListener('change', () => {
    state.wave.clueGiverId = waveClueSelect.value || null;
    if (state.currentGame === 'wavelength' && state.wave.status === 'idle') {
      renderWaveState();
    }
  });
}
