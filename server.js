...
function ensureWaveTracking(room) {
  ...
  if (!Array.isArray(room.wave.clueOrder)) {
    room.wave.clueOrder = Array.isArray(room.wave.clueOrder) ? room.wave.clueOrder : [];
  }
  if (typeof room.wave.nextClueIndex !== 'number' || Number.isNaN(room.wave.nextClueIndex)) {
    room.wave.nextClueIndex = -1;
  }
  if (typeof room.wave.roundCounter !== 'number' || Number.isNaN(room.wave.roundCounter)) {
    room.wave.roundCounter = 0;
  }
}
...
function prepareWaveClueOrder(room) {
  ensureWaveTracking(room);
  if (!room.players || !(room.players instanceof Map)) {
    return [];
  }
  const players = Array.from(room.players.keys());
  let order = Array.isArray(room.wave.clueOrder)
    ? room.wave.clueOrder.filter((id) => players.includes(id))
    : [];

  const missing = players.filter((id) => !order.includes(id));
  ...
  room.wave.clueOrder = order;

  if (typeof room.wave.nextClueIndex !== 'number' || Number.isNaN(room.wave.nextClueIndex)) {
    room.wave.nextClueIndex = -1;
  } else if (order.length === 0) {
    room.wave.nextClueIndex = -1;
  } else if (room.wave.nextClueIndex >= order.length) {
    room.wave.nextClueIndex = order.length - 1;
  }

  return order;
}

function pickNextWaveClueGiver(room) {
  const order = prepareWaveClueOrder(room);
  if (order.length === 0) {
    return null;
  }
  room.wave.nextClueIndex = (room.wave.nextClueIndex + 1) % order.length;
  return order[room.wave.nextClueIndex];
}
...
io.on('connection', (socket) => {
  ...
  socket.on('startWaveRound', ({ roomId, adminToken }) => {
    const room = rooms.get(roomId);
    if (!room || room.adminToken !== adminToken) return;
    ...
    const clueGiverId = pickNextWaveClueGiver(room);
    if (!clueGiverId || !room.players.has(clueGiverId)) {
      socket.emit('waveError', 'Impossible de choisir qui fait deviner.');
      return;
    }
  ...
  socket.on('disconnect', () => {
    ...
    if (Array.isArray(room.wave.clueOrder)) {
      const index = room.wave.clueOrder.indexOf(socket.id);
      if (index !== -1) {
        room.wave.clueOrder.splice(index, 1);
        if (room.wave.clueOrder.length === 0) {
          room.wave.nextClueIndex = -1;
        } else if (typeof room.wave.nextClueIndex === 'number' && !Number.isNaN(room.wave.nextClueIndex)) {
          if (index <= room.wave.nextClueIndex) {
            room.wave.nextClueIndex = Math.max(-1, room.wave.nextClueIndex - 1);
          }
          if (room.wave.nextClueIndex >= room.wave.clueOrder.length) {
            room.wave.nextClueIndex = room.wave.clueOrder.length - 1;
          }
        }
      }
    }
